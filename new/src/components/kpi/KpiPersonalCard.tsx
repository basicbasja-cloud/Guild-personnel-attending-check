...
import type { KpiProfileRow, KpiRoleTag, KpiMetricKey, KpiProgressiveState } from '../../types';
...
// ── Rank Badge ────────────────────────────────────────────────────────────────
function RankBadge({ tier }: { tier: 1 | 2 | 3 }) {
  const cfg = RANK_TIER_CONFIG[tier];
  const icons: Record<number, string> = { 3: '🏆', 2: '⚡', 1: '🌊' };
  return (
...
function ProgressCell({
  ...
  const state = row.progressive_state as KpiProgressiveState;
...
export function KpiPersonalCard({ userId, weekStart }: KpiPersonalCardProps) {
  ...
  const weekRows = useMemo(
    () => rows.filter((r) => r.week_start === weekStart),
    [rows, weekStart],
  );

  // Derive role and rank from first row (all rows for same week share role)
